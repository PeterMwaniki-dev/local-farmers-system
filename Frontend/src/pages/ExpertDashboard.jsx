      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-md p-6`}>
          <div className="flex justify-between items-center mb-6">
            <h2 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'}`}>My Advisory Posts</h2>
            <Link
              to="/advisory/posts/create"
          ...
          {loading ? (
            <div className="text-center py-12">
              ...
              <p className={`mt-4 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Loading your posts...</p>
            </div>
          ) : error ? (
            <div className="bg-yellow-100 dark:bg-yellow-900 border border-yellow-400 dark:border-yellow-800 text-yellow-700 dark:text-yellow-300 px-4 py-3 rounded">
            ...
            <div className="space-y-4">
              {posts.map((post) => (
                <div key={post.post_id} className={`border rounded-lg p-4 hover:shadow-md transition ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex-1">
                      <h3 className={`text-lg font-semibold mb-1 ${darkMode ? 'text-white' : 'text-gray-800'}`}>{post.title}</h3>
                      {post.category && (
                        <span className="inline-block bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300 text-xs px-2 py-1 rounded">
                          {post.category}
                        </span>
                      )}
                    </div>
